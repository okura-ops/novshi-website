...
取得実施日: 2026-07-09 ／ 対象14ツール（Google Workspaceを4製品に分解）
...
**⚠️ 公開前に大倉さんの確認が必要（利用可否＝要判断）**: anthropic, google-drive, gmail, google-calendar, google-meet, github, cloudsign（下表「利用可否」列を参照）。**明確にOK**: vercel, manus, notta（notta=2026-07-10 メール許諾受領）。
...
| **google-drive** | ✅取得済 | https://knowledge.workspace.google.com/admin/getting-started/brand-your-internal-communications-with-google-workspace | 2026-07-09 | SVG+PNG | drive.svg, logo_drive_2026_color_512.png | 最小:規定なし／クリアスペース:規定なし／改変:リサイズのみ可・再配色や他変更不可・ワードマーク結合不可／モノクロ:不可（フルカラーのみ） | **要判断**（当該ページの許諾は「組織の社内向け資料」スコープ。対外Webマップは外部マーケ用途で明示的許諾外。Google商標は一般に許諾要） |
| **gmail** | ✅取得済 | 同上（Workspace知識ベース） | 2026-07-09 | SVG | gmail.svg | 最小:規定なし／クリアスペース:規定なし／改変:ブランド要素の除去・変更不可・ワードマーク結合不可／モノクロ:不可 | **要判断**（Drive同様=社内向けスコープ、対外用途は許諾外） |
| **google-calendar** | ✅取得済 | 同上（Workspace知識ベース） | 2026-07-09 | SVG | calendar.svg | 最小:規定なし／クリアスペース:規定なし／改変:ブランド要素の除去・変更不可／モノクロ:不可 | **要判断**（同上） |
| **google-meet** | ✅取得済 | 同上（Workspace知識ベース） | 2026-07-09 | SVG | meet.svg | 最小:規定なし／クリアスペース:規定なし／改変:ブランド要素の除去・変更不可／モノクロ:不可 | **要判断**（同上） |
...
## Chrome委譲リスト（後日、ブラウザで取得＋規約確認）

いずれも「JSレンダリングSPA」または「規約同意ゲート」のため、curl/自動fetchでは取得できませんでした。ブラウザでページを開き、①規約を読み ②連携マップ用途の可否を確認（必要なら下記連絡先へ許諾依頼）③SVG/PNGをDL、の順で処理してください。
...
> notta は 2026-07-10 に media-kit（notta.ai/media-kit）から手動取得済み（notta-app-logo.png）。上表からは削除。限定配布での連携マップ掲載は 2026-07-10 に contact@notta.ai からメール許諾を受領済み（利用可否＝明確にOK）。
...
| ツール | 理由 | 代替案 |
|:---|:---|:---|
| notion | 公式が第三者のロゴ単独利用を明示禁止 | 自社タイプで「Notion」ワードマーク、または許容表現の記述 |
