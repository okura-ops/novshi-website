...
## design-assets/vendor-logos/（対外用ツール連携マップ専用）

- 「novshiが利用するツールの連携マップ（対外用）」制作のための**公式ロゴアセット置き場**。`design-assets/` は `public/` 配下ではないため**ビルド成果物・本番配信対象に含まれない**（マップ制作＝Claude Design専用）。
- **`design-assets/` はデザイン制作専用。`public/` へコピーしてのサイト掲載は禁止**（各社の利用規約要確認のため。掲載可否は大倉さんの判断を経る）。
- 各社の**原本を無改変で格納**（再配色・トリミング・変形・他マーク結合は禁止）。出所・取得日・形式・ガイドライン要点・利用可否の台帳は `design-assets/vendor-logos/README.md` が正。
- 「要判断」ツール（Google各製品・GitHub・CloudSign・Anthropic等）は対外掲載前に大倉さんの確認（必要に応じ各社への許諾確認）を経ること。
